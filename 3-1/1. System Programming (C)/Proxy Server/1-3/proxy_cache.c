...
int main(int argc, char* argv[])
{
    ...
    while(1)
    {
        printf("[%d]input CMD> ", getpid());    // input CMD
        scanf("%s", cmd);

        if(strcmp(cmd, "quit") == 0)            // if quit
        {
            main_end = time(NULL);            // measure time

            fprintf(log_file, "**SERVER** [Terminated] run time: %d sec. #sub process: %d\n",
                    main_end - main_start, p_count);
            exit(0);                            // exit
        }
        else if(strcmp(cmd, "connect") == 0)   // if connect
        {
        ...
        else if(PID == 0)  // if PID = 0, Child process
        {
            ...
            h_count = 0, m_count = 0;       // initialization
            ...
            printf("[%d]input URL> ", getpid());      // input URL
            scanf("%s", input);
        ...
        {
            printf("please input 'connect' or 'quit'\n");
            continue;
        }
    }
    /*
