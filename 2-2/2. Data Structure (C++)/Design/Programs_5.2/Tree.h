...
public:
	...
	void Preorder()                                             //Preorder 함수 선언
	{
		Preorder(root);                                         //처음 Preorder 함수로 들어갈 때, 입력에 root를 넣어주도록 설정
	}

	void Preorder(TreeNode<T> *currentNode)                     //재귀함수를 통하여 Preorder순회방법 구현
	{
		if (currentNode)                                        
		{
			Visit(currentNode);                                 //currentNode의 입력형태로하는 Visit 함수 호출
			Preorder(currentNode->leftChild);                   //currentNode의 leftChild를 입력형태로하는 Preorder함수 호출
			Preorder(currentNode->rightChild);					//currentNode의 rightChild를 입력형태로하는 Preorder함수 호출
		}
