...
public:
	...
	void Postorder()										    //Postorder 함수 선언
	{
		Postorder(root);									    //처음 Postorder 함수로 들어갈 때, 입력에 root를 넣어주도록 설정
	}

	void Postorder(TreeNode<T> *currentNode)				    //재귀함수를 통하여 Postorder 순회방법 구현
	{
		if (currentNode)							
		{
			Postorder(currentNode->leftChild);			    	//currentNode의 leftChild를 입력 형태로하는 Postorder함수 호출
			Postorder(currentNode->rightChild);				    //currentNode의 rightChild를 입력 형태로하는 Postorder함수 호출
			Visit(currentNode);								    //currentNode를 입력 형태로하는 Visit 함수 호출
		}
	}
