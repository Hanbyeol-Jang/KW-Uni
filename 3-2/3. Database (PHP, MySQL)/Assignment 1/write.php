<?php
	...
	if(isset($bNo)) {
        $sql = "select b_title,b_content,b_id
                from board_free
                where b_no = '$bNo'";
		$result = $db->query($sql);
...
<head>
    <meta charset="utf-8"/>
    <title>자유게시판 글쓰기 | Hanbyeol's Library</title>
                ...
                <table id="boardWrite">
                    ...
                    <tbody>
                        ...
                        <tr>                                
                            ...
                            <td class="content"><textarea name="bContent" id="bContent"><?php echo isset($row['b_content'])?$row['b_content']:null?></textarea>
                            </td>
                        </tr>
                        <tr>                                
                            <th scope="row"><label for="bImage">이미지</label></th>
                            <td class="image">
                            <input type="file" name="bImage" id="bImage"
                                value="<?php echo isset($row['b_image'])?$row['b_image']:null?>">
                            </td>
                        </tr>
                    </tbody>
                </table>
